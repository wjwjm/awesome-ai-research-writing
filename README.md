...
#### **2. 文章脉络与逻辑链 (Logic Flow)**
请按以下结构梳理文章的推导逻辑：
* **物理猜想 (Hypothesis)**：作者预期的非线性过程或物理图像是什么？
* **研究路径 (Methodology)**：
    * **理论/模拟**：是否引入了特殊的电离模型？使用了什么传播方程（如 UPPE, NLSE）或 PIC 代码？
    * **实验设计**：如何实现精密的时空重合？采用了哪些关键诊断手段（如 FROG, SPIDER, 或远场干涉诊断）？
* **证据链条 (Evidence Chain)**：识别关键图表（Figure X），说明实验观测现象（如丝核演化、频谱蓝移、干涉条纹对比度）是如何层层递进支撑核心结论的。

#### **3. 核心物理参数提取 (Technical Parameters - 严格事实核查)**
请以表格形式列出以下参数。**注意：仅填写文中明确给出的数值，未提及的标注为 "N/A"。**
* **激光/脉冲**：$\lambda$, $\tau$, $I_0$, $a_0$, 啁啾, 聚焦条件。
* **介质/环境**：介质类型、压力/密度、$I_p$、等离子体密度 $n_e$。
* **计算/实验**：代码名称、时空步长、探测器分辨率。
